...
class PBrute {
  ...
  calculate(password) {
    ...
    if (longestSuffix.length > 4)
      messages.push({type: 'pattern', text: `${this.translate('pattern.long')}`});
    else if (longestSuffix.length > 1)
      messages.push({type: 'pattern', text: `${this.translate('pattern.short')}`});
    if (longestSuffix.length > 1 && suffixOccurances > 2)
      messages.push({type: 'pattern', text: `${this.translate('pattern.repeating')}`});
    else if (longestSuffix > 0 && suffixOccurances > 2)
      messages.push({type: 'pattern', text: `${this.translate('pattern.repeatingCharacter')}`});
  ...
  calculateCharacterSet(password) {
    ...
    // Anything else
    const others = password.match(/[^\w !"#$%&'()*+,-./:;<=>?@[\\\]^_`{|}~áÁàÀâÂäÄãÃåÅæÆçÇéÉèÈêÊëËíÍìÌîÎïÏñÑóÓòÒôÔöÖõÕøØœŒßúÚùÙûÛüÜ]/g);
  ...
  humanizeTime(time) {
    const reducedTime = this.reduceTime(time);
    const roundedTime = this.roundTime(reducedTime);
    const simplifiedTime = this.simplifyTime(roundedTime);
  ...
  async haveIBeenPwnd(password) {
    const digest = (await sha1(password)).toUpperCase();
